...
try {
    ...
    # ------------------------------------------------------------
    $ImageName = "word-processing-layer"
    $Container = "word-layer-temp"
    $OptDir    = "opt"
    $ZipFile   = "word-processing-layer.zip"
    ...
    Write-Host ""
    Write-Host "SUCCESS: DOCX Lambda layer created -> $ZipFile" -ForegroundColor Green
}
catch {
    Write-Host ""
